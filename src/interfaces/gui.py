...
def _render_model_selection_sidebar(chat_session: ChatSession) -> Tuple[str, str]:
    ...
    all_models_structured = [
        ...
        ("OpenAI & Gemini (Collab)", "a2a_collab_v1", "collab") # Added collab option
    ]
    model_display_names = [item[0] for item in all_models_structured if item[0]] # Filter out empty names
    ...
    if model_display_names: # Ensure there are models to choose from
        ...
        if chat_session.openai_manager.available and chat_session.openai_model in model_display_names:
            initial_default_model_name = chat_session.openai_model
        elif chat_session.gemini_manager.available and chat_session.gemini_model in model_display_names:
    ...
    current_selected_model_name = st.session_state.get("selected_model_name", initial_default_model_name)

    if not model_display_names :
        current_selected_model_name = "" 
        default_idx = 0 
    elif current_selected_model_name not in model_display_names:
        if initial_default_model_name and initial_default_model_name in model_display_names:
    ...
    old_selected_model_in_session = st.session_state.get("selected_model_name_in_session_state_before_selectbox", None)
    ...
    if old_selected_model_in_session != chosen_display_model or old_active_provider_in_session != newly_selected_provider:
        logger.info(f"Model selection changed. New model: {chosen_display_model}, New provider: {newly_selected_provider}. Previous model in session: {old_selected_model_in_session}, Previous provider: {old_active_provider_in_session}")

    st.session_state.selected_model_name_in_session_state_before_selectbox = chosen_display_model
    ...
    if newly_selected_provider == "openai":
        ...
        if not hasattr(chat_session, 'gemini_model') or chat_session.gemini_model not in gemini_models_list:
            if gemini_models_list: 
                chat_session.gemini_model = config.DEFAULT_GEMINI_MODEL if config.DEFAULT_GEMINI_MODEL in gemini_models_list else gemini_models_list[0]
        ...
        if not hasattr(chat_session, 'openai_model') or chat_session.openai_model not in openai_models_list:
            if openai_models_list: 
                chat_session.openai_model = config.DEFAULT_OPENAI_MODEL if config.DEFAULT_OPENAI_MODEL in openai_models_list else openai_models_list[0]
        ...
        logger.info(f"Collaboration mode selected. Specific model for OpenAI part of collab: {chat_session.openai_model}, for Gemini part: {chat_session.gemini_model}")
...
def _render_api_status_sidebar(chat_session: ChatSession):
    ...
    openai_status = "✅ Connected" if chat_session.openai_available else (
        "❌ (no API key)" if not config.OPENAI_API_KEY else "❌ (unavailable)")
    gemini_status = "✅ Connected" if chat_session.gemini_available else (
        "❌ (no API key)" if not config.GOOGLE_API_KEY else "❌ (unavailable)")
...
def _update_and_display_token_counts(chat_session: ChatSession):
    ...
    if "current_user_message_tokens_log" in st.session_state:
        for count, provider, _model_name in st.session_state.current_user_message_tokens_log:
            if provider == "openai": user_openai_tokens += count
            elif provider == "gemini": user_gemini_tokens += count
    ...
    for sender, content in chat_session.chat_log: 
        if sender == "openai":
            ...
            else: agent_openai_tokens += len(content.split()) 
        elif sender == "gemini":
            ...
            else: agent_gemini_tokens += len(content.split())

    st.session_state.current_conversation_openai_tokens = user_openai_tokens + agent_openai_tokens
    st.session_state.current_conversation_gemini_tokens = user_gemini_tokens + agent_gemini_tokens


def _render_token_counts_sidebar():
    """Renders the token count display in the sidebar."""
    if "chat_session" in st.session_state: # Ensure counts are calculated if session exists
    ...
    st.sidebar.markdown(f"**Last Input:** {st.session_state.get('last_user_input_token_count', 0)} tokens ({last_provider_display})")
...
def _render_clear_chat_button() -> bool:
    ...
    if st.sidebar.button("Clear Chat"):
        logger.info("Clear Chat button clicked. Resetting chat session and token counts.")
        st.session_state.all_time_total_openai_tokens += st.session_state.current_conversation_openai_tokens
        st.session_state.all_time_total_gemini_tokens += st.session_state.current_conversation_gemini_tokens
...
def _render_input_form(chat_session: ChatSession, selected_model_name_from_sidebar: str, selected_provider_from_sidebar: str):
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        if submitted and user_message.strip():
            with st.spinner("Waiting for response..."):
                active_provider = st.session_state.active_provider # This should be the one set by model selector
                
                logger.debug(f"Form submitted. Active provider: {active_provider}, Model for processing: {selected_model_name_from_sidebar}, User message: '{user_message[:50]}...'")

                current_input_tokens = 0
                if active_provider == "openai":
                    if chat_session.openai_manager and chat_session.openai_manager.available:
                        current_input_tokens = chat_session.openai_manager.count_tokens(user_message)
                    else: current_input_tokens = len(user_message.split()) 
                elif active_provider == "gemini":
                    if chat_session.gemini_manager and chat_session.gemini_manager.available:
                        current_input_tokens = chat_session.gemini_manager.count_tokens(user_message)
                    else: current_input_tokens = len(user_message.split()) 
                ...
                st.session_state.current_user_message_tokens_log.append(
                    (current_input_tokens, active_provider, selected_model_name_from_sidebar) 
                )

                chat_session.process_user_message(user_message, model_choice=active_provider, specific_model_name=selected_model_name_from_sidebar)
                logger.debug("Finished processing user message, preparing to update token counts and rerun.")
...
def main_gui():
    ...
    if "chat_session" not in st.session_state:
        ...
        st.session_state.all_time_total_openai_tokens = 0
        st.session_state.all_time_total_gemini_tokens = 0
        logger.info("New ChatSession and token states initialized for Streamlit session.")

    chat_session_instance = st.session_state.chat_session

    # Sidebar rendering
    selected_model, active_provider = _render_model_selection_sidebar(chat_session_instance)
    ...
    if _render_input_form(chat_session_instance, selected_model, active_provider):
        ...
        _update_and_display_token_counts(chat_session_instance) # Explicit call after processing
        st.rerun()
