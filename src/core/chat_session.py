...
from src.handlers.command import CommandHandler, Command, CommandType # Updated import
...
class Message:
    ...
    sender_provider: Optional[str] = None  # e.g., "user", "system", "openai", "gemini", "collab"

class ConversationHistory:
    def __init__(self, system_prompt_content: str):
        self.messages: List[Message] = []
        if system_prompt_content:
            logger.debug("Initializing ConversationHistory with system prompt.") # Added log
            self.add_message(role="system", content=system_prompt_content, sender_provider="system")

    def add_message(self, role: str, content: str, sender_provider: Optional[str]):
        logger.debug(f"Adding message to history: Role={role}, Provider={sender_provider}, Content='{content[:50]}...' ") # Added log
        self.messages.append(Message(role=role, content=content, sender_provider=sender_provider))
...
class ChatSession:
    ...
    def __init__(self):
        ...
        self.openai_manager = OpenAIClientManager(api_key=openai_api_key, default_model_name=default_openai_model)
        self.gemini_manager = GeminiClientManager(api_key=google_api_key, default_model_name=default_gemini_model)
        
        self.file_tool = FileManagerTool() 
        self.command_handler = CommandHandler(file_tool=self.file_tool) # Changed and initialized with file_tool
        
        # Initialize ConversationHistory
        # System prompt could also come from config.py if desired: config.DEFAULT_SYSTEM_PROMPT
        self.system_prompt = ("You are a helpful AI assistant. You have access to a local project file system. "
    ...
    def process_user_message(self, user_input: str, model_choice: str = "openai", specific_model_name: str = None, use_a2a: bool = False):
        ...
        """
        processed_user_input = user_input.strip() # Renamed to avoid conflict with original user_input
        ...
        if specific_model_name:
            if model_choice == "openai":
                ...
                if success:
                    ...
                    logger.warning(f"Attempt to change Gemini model to {specific_model_name} encountered issues (see previous logs from GeminiClientManager).")
        
        self.last_model = model_choice
        logger.debug(f"Processing user message. Model choice: {model_choice}, Last model: {self.last_model}, User input: '{processed_user_input[:50]}...' ") # Added log

        self.history.add_message(role="user", content=processed_user_input, sender_provider="user")

        # Check and handle file system commands using the new CommandHandler
        output_messages = []
        sender_for_commands = model_choice # "openai" or "gemini" - for attributing command output

        parsed_command: Optional[Command] = self.command_handler.parse(processed_user_input, self.history.get_chat_log())

        if parsed_command:
            logger.info(f"Parsed command: {parsed_command.command_type} with args: {parsed_command.args}") # Added log
            command_response_text = self.command_handler.execute_command(parsed_command, self, processed_user_input)
            
            if command_response_text:
                logger.info(f"Command response generated: '{command_response_text[:100]}...' ") # Added log
                self.history.add_message(role="assistant", content=command_response_text, sender_provider=sender_for_commands)
        ...
        if model_choice == "both" or model_choice == "collab": # "both" from CLI, "collab" from GUI
            logger.info(f"Attempting A2A collaboration for input: '{processed_user_input[:50]}...'")
            if self.openai_manager.available and self.gemini_manager.available:
                ...
                if self.openai_manager.client and self.gemini_manager.client:
                    ...
                    self.history.add_message(role="assistant", content=collab_answer, sender_provider="collab")
                    output_messages.append(("collab", collab_answer))
                    logger.info(f"A2A collaboration successful. Response: '{collab_answer[:100]}...'")
                else:
                    error_msg = "⚠️ A2A Collaboration failed: one or both LLM clients are not properly initialized internally."
                    logger.error(error_msg)
                    self.history.add_message(role="assistant", content=error_msg, sender_provider="system") # Or "collab_error"
        ...
        if model_choice == "openai":
            ...
            if not self.openai_manager.available:
                ...
                self.history.add_message(role="assistant", content=error_msg, sender_provider="openai")
                output_messages.append(("openai", error_msg))
                return output_messages
            
            logger.debug(f"Sending request to OpenAI model: {self.openai_manager.get_model_name()}")
            answer = self.openai_manager.generate_response(self.history.get_openai_format())
            logger.debug(f"Received answer from OpenAI: '{answer[:100]}...' ")
            self.history.add_message(role="assistant", content=answer, sender_provider="openai")
            ...
            if not self.gemini_manager.available:
                ...
                self.history.add_message(role="assistant", content=error_msg, sender_provider="gemini")
                output_messages.append(("gemini", error_msg))
                return output_messages

            logger.debug(f"Sending request to Gemini model: {self.gemini_manager.get_model_name()}")
            answer = self.gemini_manager.generate_response(self.history.get_gemini_format())
            logger.debug(f"Received answer from Gemini: '{answer[:100]}...' ")
            self.history.add_message(role="assistant", content=answer, sender_provider="gemini")
    ...
    def confirm_overwrite(self):
        ...
        tool_input = ToolInput(operation_name="write", 
                               args={"path": self.pending_write_user_path, 
        ...
        if tool_output.success:
            ...
            response_text = tool_output.error or tool_output.message or "⚠️ Error during GUI overwrite."
        
        sender_label = self.last_model if self.last_model in ["openai", "gemini"] else "openai"
        self.history.add_message(role="assistant", content=response_text, sender_provider=sender_label)
        logger.info(f"Overwrite confirmed via GUI for path: {self.pending_write_user_path or 'N/A'}, result: {response_text}")
