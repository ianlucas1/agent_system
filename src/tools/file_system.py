import os
from typing import Dict, Any, Optional # List removed as it's not used here
...
def _resolve_path(user_path: str) -> str:
    ...
    if not abs_path.startswith(PROJECT_ROOT_DIR):
        raise ValueError(f"⚠️ Access denied: path `{user_path}` is outside the allowed project directory.")
    return abs_path

def read_file_content(path: str, max_bytes: int = 100*1024, max_lines: int = 200) -> str:
    ...
    def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        try:
            if op_name == "read":
                path = args.get("path")
                if path is None:
                    return ToolOutput(success=False, error="Path not provided for read operation.")
                ...
                return ToolOutput(success=True, message=formatted_content, data={"raw_content": content})

            elif op_name == "list":
                path = args.get("path", ".")
                listing = list_directory_contents(path) # Uses co-located function
                formatted_listing = f"Contents of `{path}`:\n{listing}"
                return ToolOutput(success=True, message=formatted_listing, data={"listing_text": listing})
                ...
                if path is None: return ToolOutput(success=False, error="Path not provided for write operation.")
                if content is None: return ToolOutput(success=False, error="Content not provided for write operation.")
                ...
                bytes_written = write_content_to_file(path, content) # Uses co-located function
                return ToolOutput(
                    success=True, 
                ...
                return ToolOutput(success=False, error=f"Unknown operation for FileManagerTool: {op_name}")

        except (FileNotFoundError, IsADirectoryError, NotADirectoryError, ValueError, OSError) as e_fm:
            return ToolOutput(success=False, error=str(e_fm))
        except Exception as e_general:
            return ToolOutput(success=False, error=f"An unexpected error occurred in FileManagerTool: {e_general}") 