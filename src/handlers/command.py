...
class CommandHandler:
    def __init__(self, file_tool: FileManagerTool):
        self.write_nl_re = re.compile(r'^(?:save|write)\s+(?:this|that)\s+to\s+(.+)$', re.IGNORECASE)
        self.file_tool = file_tool

    def parse(self, user_input: str, chat_log_for_context: List[Tuple[str, str]]) -> Optional[Command]:
        """
        ...
        if stripped_input.startswith("/"):
            ...
            if cmd_token == "/read":
                ...
                dir_path = parts[1].strip() if len(parts) >= 2 and parts[1].strip() else "."
                return Command(CommandType.LIST, args=dir_path)
            elif cmd_token == "/write":
                if len(parts) < 3:
                    return Command(CommandType.UNKNOWN, args="Usage: /write <filepath> <content>")
                return Command(CommandType.WRITE, args=(parts[1].strip(), parts[2]))
            elif cmd_token == "/overwrite":
                if len(parts) < 2 or not parts[1].strip():
                    return Command(CommandType.UNKNOWN, args="Usage: /overwrite <filename>")
                return Command(CommandType.OVERWRITE, args=parts[1].strip())
            else:
                return Command(CommandType.UNKNOWN, args=f"Unknown command: {cmd_token}")
        ...
        if low_input.startswith("what's in") or low_input.startswith("what is in") or low_input.startswith("list "):
            dir_query = re.sub(r"^(what's in( the)?|what is in( the)?|list)\s*", "", stripped_input, flags=re.IGNORECASE).strip(" ?.folderdirectory")
            return Command(CommandType.LIST, args=dir_query if dir_query else ".")

        if low_input.startswith("read ") or low_input.startswith("open ") or low_input.startswith("show me ") or low_input.startswith("show "):
            if not (" folder" in low_input or " directory" in low_input):
                file_query = re.sub(r"^(read( the)?|open( the)?|show me( the)?|show)\s*", "", stripped_input, flags=re.IGNORECASE).strip(" ?")
                if file_query:
                    return Command(CommandType.READ, args=file_query)
        return None

    def execute_command(self, parsed_command: Command, chat_session: 'ChatSession', current_user_input: str) -> Optional[str]:
        """
        ...
        """
        command_response_text: Optional[str] = None
        tool_output = None

        try:
            if parsed_command.command_type == CommandType.READ:
                path = parsed_command.args
                logger.debug(f"Executing READ command for path: {path}") # Added debug log
                ...
                logger.debug(f"Executing LIST command for path: {dir_path}") # Added debug log
                ...
                logger.debug(f"Attempting WRITE command for path: {path}") # Added debug log
                
                if content_to_write is None: # Specific to Natural Language "save this to file.txt"
                    ...
                    for msg_obj in reversed(chat_session.history.messages):
                        if msg_obj.role == "user" and msg_obj.content == current_user_input:
                            continue 
                        if msg_obj.role == "assistant" and msg_obj.sender_provider in ("openai", "gemini", "collab"):
                            last_assistant_message = msg_obj.content
                            break
                    if last_assistant_message is None:
                        logger.warning("NL Write command: No previous assistant message found to save.") # Added log
                        return "⚠️ No previous assistant message found to save." # Return directly
                    content_to_write = last_assistant_message
                    logger.debug(f"NL Write command: Found content to save for path {path}") # Added log
                
                tool_input = ToolInput(operation_name="write", 
                                       args={"path": path, "content": content_to_write, "allow_overwrite": False})
                tool_output = self.file_tool.execute(tool_input)
                
                if not tool_output.success and tool_output.data and tool_output.data.get("status") == "overwrite_required":
                    chat_session.pending_write_user_path = path 
                    chat_session.pending_write_content = content_to_write
                    logger.info(f"Write command for path {path} requires overwrite confirmation.") # Added log
                ...
                logger.debug(f"Executing OVERWRITE command for file: {target_file_basename}") # Added debug log
                if chat_session.pending_write_user_path is None or chat_session.pending_write_content is None:
                    logger.warning("Overwrite command: No pending write operation to confirm.") # Added log
                    return "⚠️ No pending write operation to confirm." # Return directly
                
                pending_filename = os.path.basename(chat_session.pending_write_user_path)
                if target_file_basename != pending_filename:
                    logger.warning(f"Overwrite command: Mismatch. Target: {target_file_basename}, Pending: {pending_filename}") # Added log
                    return f"⚠️ No pending overwrite for `{target_file_basename}` (pending file is `{pending_filename}`)."
                
                tool_input = ToolInput(operation_name="write", 
                                       args={"path": chat_session.pending_write_user_path, 
                ...
                if tool_output.success:
                    ...
                    logger.info(f"Overwrite successful for {pending_filename}") # Added log
            
            elif parsed_command.command_type == CommandType.UNKNOWN:
                logger.warning(f"Unknown command encountered: {parsed_command.args}") # Added log
            ...
            if tool_output:
                if tool_output.success:
                    ...
                    logger.error(f"Tool execution failed. Operation: {tool_output.data.get('operation', 'N/A') if tool_output.data else 'N/A'}. Error: {tool_output.error}. Message: {tool_output.message}")
                    return tool_output.message or tool_output.error or "⚠️ An unknown error occurred during tool operation."
            ...
            if parsed_command.command_type not in [CommandType.UNKNOWN]: # UNKNOWN already returned
                 # This case should ideally not be reached if all command types are handled.
                 logger.error(f"Command {parsed_command.command_type} was recognized but not fully processed leading to no tool_output.")
            ...
            logger.exception(f"Unexpected error in CommandHandler.execute_command for command {parsed_command.command_type if parsed_command else 'N/A'}") # Changed to logger.exception
